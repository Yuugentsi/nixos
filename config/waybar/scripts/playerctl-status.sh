...
if [ -n "$artist" ] && [ -n "$title" ]; then
    if [ -n "$art_url" ] && command -v wal >/dev/null && command -v jq >/dev/null; then
        # Usa wal para extrair a cor de destaque da capa do álbum
        wal -s -n -i "$art_url" >/dev/null 2>&1
        ACCENT_COLOR=$(jq -r '.colors.color1' "$WAL_COLORS_FILE" 2>/dev/null)

        if [ -z "$ACCENT_COLOR" ]; then
            ACCENT_COLOR="#1DB954" # Cor padrão se wal falhar
        fi
    else
        ACCENT_COLOR="#1DB954" # Cor padrão se não houver art_url ou wal/jq
    fi
    ...
    # Definindo as variáveis sem limitar o tamanho para permitir o scrolling no Waybar
