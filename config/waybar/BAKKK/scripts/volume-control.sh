...
# Define functions
print_error() {
  echo "Usage: ./volumecontrol.sh -[device] <action> [step]"
  echo "Devices: i (input), o (output), p (player)"
  echo "Actions: i (increase volume by step or 10%), d (decrease volume by step or 10%), m (mute)"
  exit 1
}
...
action_volume() {
  step=${2:-10}  # Default to 10 if no step provided
  case "${1}" in
  i)
    # Increase volume if below 90
    current_vol=$(pactl get-sink-volume @DEFAULT_SINK@ | awk '{print $5}' | sed 's/%//')
    new_vol=$((current_vol + step))
    [ "$new_vol" -gt 90 ] && new_vol=90 # Modificado para limitar a 90%
